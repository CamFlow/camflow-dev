...
static __always_inline struct provenance *provenance_alloc_with_ipv6_skb(uint64_t type, struct sk_buff *skb)
{
	...
	switch (ih->nexthdr) {
	case IPPROTO_TCP:
		__extract_ipv6_tcp_info(skb, ih, offset, &ipv6_packet_identifier(prov_elt(prov)));
		break;
	case IPPROTO_UDP:
		__extract_ipv6_udp_info(skb, ih, offset, &ipv6_packet_identifier(prov_elt(prov)));
		break;
	default:
		break;
	}
...
typedef struct ipv6_filters ipv6_filters;
...
static inline uint8_t prov_ipv6_whichOP(struct list_head *filters, struct in6_addr ip, uint16_t port)
{
	struct list_head *listentry, *listtmp;
	ipv6_filters *tmp;
